"""
Plotting: titration curves for individual fits, showing DMS reactivity
vs. pH with fitted Henderson-Hasselbalch curves overlaid.
"""
...
def _get_condition_data(df_res, pka, name, pos, buffer, temp, mg):
    ...
    pka_mask = (
        (pka["name"] == name) & (pka["pos"] == pos)
        & (pka["buffer_conc"] == buffer) & (pka["temperature"] == temp)
        & (pka["mg_conc"] == mg) & (pka["pKa"].notna())
    )
    pka_subset = pka.loc[pka_mask]
    if len(pka_subset) == 0:
        return None
...
def _collect_position_data(pka, df_res, motif_pos_idx, conditions):
    """Collect data for all constructs at one motif position."""
    pos_df = pka[(pka["motif_pos_idx"] == motif_pos_idx) & (pka["pKa"].notna())]
    result = {}
    for name in pos_df["name"].unique():
        pos = pos_df[pos_df["name"] == name].iloc[0]["pos"]
        cond_list = []
        for buffer, temp, mg, color, label in conditions:
            cond_data = _get_condition_data(df_res, pka, name, pos, buffer, temp, mg)
            if cond_data:
...
def _count_total(pka, motif_pos_idx, conditions):
    """Count total constructs per condition (for denominator display)."""
    pos_df = pka[pka["motif_pos_idx"] == motif_pos_idx]
    counts = {}
...
def plot_titration_curves(
    ...
    pka_unfiltered: pd.DataFrame | None = None,
):
    ...
    df_res : pd.DataFrame
        Per-residue data.
    pka : pd.DataFrame
        Annotated pKa DataFrame (filtered or unfiltered).
    ...
    pka_unfiltered : pd.DataFrame, optional
        If provided, shown construct counts include total/denominator.
    """
    all_motifs = pka["motif_sequence"].dropna().unique()
    ...
    for motif_seq in motifs:
        ...
        pka_sub = pka[pka["motif_sequence"] == motif_seq].copy()
        pka_sub["motif_pos_idx"] = pka_sub.apply(
            lambda r: (
        ...
        pka_unfilt_sub = None
        if pka_unfiltered is not None:
            pka_unfilt_sub = pka_unfiltered[pka_unfiltered["motif_sequence"] == motif_seq].copy()
            pka_unfilt_sub["motif_pos_idx"] = pka_unfilt_sub.apply(
                lambda r: (
                    r["motif_positions"].index(r["pos"])
                    if r["motif_positions"] and r["pos"] in r["motif_positions"]
                    else None
        ...
        for idx, res in enumerate(residues):
            ...
            if pka_unfilt_sub is not None:
                ...
                count_parts = [f"{label}: {filtered_counts[label]}" for *_, label in conditions]
